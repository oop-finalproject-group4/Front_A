...
//			data.chargeCount = API.getResultObject().getInt("couponCount");
			couponLabel.setText("優惠券 ：          " + data.chargeCount + "張");
...
//		System.out.println("newSite[0] :" + data.lat + ", newSite[1] :" + data.lng);
	...
	public void setrentinfo() {
		...
		bill.setText("費用： " + data.totalDis * 0.3 + " 元");
		API.info();
...
//		int resCode = API.returnCar(25.053439897900585, 121.55601888363678, 0, data.charge, 0, data.couponUse);
		...
		if (resCode == 200) {
			...
			API.returnCar(data.lat, data.lng, data.totalDis, data.charge, data.chargeCount, data.couponUse);
			JSONObject errReason = API.getResultObject();
			String err = errReason.getString("error");
			switch(err) {
			case "not renting":
				Stage popupStage = new Stage();
				popupStage.initModality(Modality.APPLICATION_MODAL);

				Label messageLabel = new Label("尚未租車");
				messageLabel.setWrapText(true);
				messageLabel.setAlignment(Pos.CENTER);
				messageLabel.setTextAlignment(TextAlignment.CENTER);
			...
			case "not in range":
				...
				Label messageLabel1 = new Label("不在可還車的範圍內");
				messageLabel1.setWrapText(true);
				messageLabel1.setAlignment(Pos.CENTER);
				messageLabel1.setTextAlignment(TextAlignment.CENTER);

				Button closeButton1 = new Button("確認");
				closeButton1.setOnAction(e -> popupStage1.close());

				VBox popupRoot1 = new VBox(messageLabel1, closeButton1);
				popupRoot1.setAlignment(Pos.CENTER);
				popupRoot1.setSpacing(10);
				popupRoot1.setPadding(new Insets(20));

				Scene popupScene1 = new Scene(popupRoot1, 250, 150);
			...
			case "not enough coupon":
				...
				Label messageLabel2 = new Label("優惠券不足，無法使用");
				messageLabel2.setWrapText(true);
				messageLabel2.setAlignment(Pos.CENTER);
				messageLabel2.setTextAlignment(TextAlignment.CENTER);

				Button closeButton2 = new Button("確認");
				closeButton2.setOnAction(e -> popupStage2.close());

				VBox popupRoot2 = new VBox(messageLabel2, closeButton2);
				popupRoot2.setAlignment(Pos.CENTER);
				popupRoot2.setSpacing(10);
				popupRoot2.setPadding(new Insets(20));

				Scene popupScene2 = new Scene(popupRoot2, 250, 150);
				popupStage2.setScene(popupScene2);
				popupStage2.showAndWait();
				System.out.println("coupon not enough");
			}
		}
	}
}
